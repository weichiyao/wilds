...
class RxRx1DatasetNoisy(WILDSDatasetNoisy):
    """
    ...
    def __init__(self, version=None, root_dir='data', download=False,
                 split_scheme='official', noise_ratio=0.1, seed=0):
        ...
        # Labels
        self._y_array_orc = torch.tensor(df['sirna_id'].values)
        self._n_classes = max(df['sirna_id']) + 1
        self._y_array = torch.tensor(_flip_data(df['sirna_id'].values, noise_ratio, self._n_classes, seed))
        self._y_size = 1
