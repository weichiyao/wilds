...
class IWildCamDatasetNoisy(WILDSDatasetNoisy):
    """
        Modified iWildCam2020 dataset.
        ...
        Label (y):
            noisy y is one of 186 classes corresponding to animal species
        Oracle label (y_orc):
            oracle y is one of 186 classes corresponding to animal species
        Metadata:
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official', 
        ...
        # Labels
        self._y_array_orc = torch.tensor(df['y'].values)
        self._n_classes = max(df['y']) + 1
        self._y_size = 1
        assert len(np.unique(df['y'])) == self._n_classes 
        self._y_array = torch.tensor(_flip_data(df['y'].values, noise_ratio, self._n_classes, seed))
